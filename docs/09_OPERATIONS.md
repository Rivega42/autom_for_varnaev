...
- **continuous aggregate** `sensor_readings_hourly` — почасовая свёртка
  ...
  Дашборд датчиков переключается на неё автоматически при диапазоне больше
  48 часов (#296); до 48 часов читает сырьё. Свёртка отстаёт от сырья до часа
  (политика обновления) — на длинных диапазонах это незаметно.
