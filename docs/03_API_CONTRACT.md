...
### 3.3a Демо-«стена роликов» (#wall, демонстрационный режим)
Дашборд `/ui/wall.html` гоняет ручной видеоанализ загруженных роликов: каждый
ролик представляется «камерой»-записью (к ней крепятся ROI-зоны и тумблеры
аналитики), анализ — браузерным движком на видео и/или серверным file-заданием.
```
GET    /api/v1/clips                 → список роликов-«камер» (file/url/camera_id/zones)
POST   /api/v1/clips/upload?name=…   → загрузить видеоролик (стрим тела; admin)
DELETE /api/v1/clips/{filename}      → убрать ролик «из стены» в .trash (admin)
GET    /api/v1/artifacts?limit=&camera_id=  → список кадров-улик для ленты дашборда
```
Эндпойнты включаются только при примонтированном каталоге роликов
(`CLIPS_DIR`, оверлей `docker-compose.clips.yml`); в боевую поставку не входят.
...
интеграция выключена. Код помечен `# СТЫК-АУРА (v2)`.

**Переключатель режима (#352).** Интеграция включается переменной окружения
`AURA_INTEGRATION_ENABLED` **или** тумблером в GUI (вкладка «Объект»). Тумблер
хранится в БД (`app_config`, ключ `aura_integration_enabled`), **приоритетнее**
env и применяется **сразу, без перезапуска** (флаг читается на каждом запросе):
```
GET /api/v1/aura/status            # {"enabled": bool, "source": "db"|"env"}
PUT /api/v1/aura/status  {"enabled": true|false}   # admin; вкл/выкл интеграцию
```
...
```
GET /api/v1/integration/events?from=&to=&type=&limit=&offset=
```
Наружу отдаём **только события** журнала (не показания датчиков и не видео) — тем
же источником, что и публичный `GET /events`. Параметры `from`/`to` (ISO-8601) и
`type` необязательны; кривые даты → `422 VALIDATION_ERROR` ещё в шлюзе.
Постранично: `limit` (по умолчанию 50, макс 1000) и `offset`; за период событий
может быть больше `limit` — полное число в `data.total`, остальное забирается
смещением `offset`.
**Поведение:** при `aura_integration_enabled=false` → `501`; при `true` → `200`,
`data` = `{ items: [...событий], total }` (формат события — §3.2).

### 4.3 АУРА передаёт настройки (D.4) — реализовано за фичефлагом (#349)
```
...
```json
{
  ...
  "schedules": [
    { "camera_id": "uuid", "interval_min": 30 }
  ],
  "pipelines_enabled": ["pose_v1"]
}
```
Все три списка необязательны (по умолчанию пустые). АУРА передаёт желаемые
пороги датчиков, расписания видеоанализа и набор включённых pipeline.
**Поведение:** при `aura_integration_enabled=false` → `501`; при `true` → `200`,
`data` = принятые настройки. Пустое тело при включённом флаге → `422`.
**Важно (v1):** настройки **валидируются и сохраняются** (в `app_config`, ключ
`aura_settings`), но к живым справочникам (`thresholds`, `schedules`, тумблеры
аналитики) **не применяются автоматически**. Связка с действующими настройками и
сосуществование с конфигурацией оператора из GUI — задача v2 (`# СТЫК-АУРА v2`):
до тех пор приоритет у настроек оператора, а присланное от АУРА хранится «как
есть» и доступно для будущего применения.

### 4.4 Уведомление о готовности задания (D.5, мы → АУРА) — реализовано (#350)
Если у задания задан `callback_url`, по завершении (`done`/`failed`) **воркер
video-analytics** шлёт на него POST:
```json
{ "task_id": "uuid", "status": "done",
  "artifacts": ["/data/artifacts/2026-06-05/clip-0007_kp.json"] }
```
**Best-effort:** отправка не влияет на статус задания и не роняет воркер; при
ошибке — ретраи (`AURA_CALLBACK_RETRIES`), затем лог. Приёмник реализует АУРА:
вернуть `200`, обрабатывать идемпотентно по `task_id` (возможна повторная
доставка). Поле `events` контракта в v1 не заполняется (система хранит счётчик
событий, а не список их id) — детали АУРА забирает опросом D.2
(`GET /analysis-tasks/{id}`, поле `result`). Защита от SSRF: только http(s);
опциональный allowlist хостов `AURA_CALLBACK_ALLOWED_HOSTS`. Срабатывает
независимо от `AURA_INTEGRATION_ENABLED` (callback ставит сама АУРА в D.1, когда
интеграция уже включена).
