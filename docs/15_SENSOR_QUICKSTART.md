...
|---|---|---|---|
| **1. ESPHome Dashboard в Docker** (рекомендую) | Docker уже стоит | Веб-интерфейс, редактор YAML, сборка, USB-прошивка и OTA, ничего не ставить в Windows | Первая компиляция долгая (качает тулчейн) |
