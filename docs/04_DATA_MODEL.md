...
Потребитель (#296): дашборд «Среда помещений — датчики» — каждая панель одним
запросом (`UNION ALL` с взаимоисключающими ветками) читает сырьё на диапазонах
до 48 часов и `avg_value` свёртки на более длинных.
