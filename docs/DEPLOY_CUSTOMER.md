...
## 3. Доступ к образам (реестр)

Наши сервисы тянутся из реестра **ghcr.io** (GitHub Container Registry).

- **Если образы публичные** (мы так и сделаем по умолчанию) — **ничего делать не нужно**,
  Docker скачает их без авторизации. Переходи к §4.
- **Если образы приватные** — один раз залогинься (мы пришлём логин и токен):
  ```bash
  echo "<ТОКЕН>" | docker login ghcr.io -u <ЛОГИН> --password-stdin
  ```
  В Windows (PowerShell):
  ...
  ```
  Должно ответить `Login Succeeded`.
