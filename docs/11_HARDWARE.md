...
| Воздух | **SHT41 / SHT40 / SHT30** | температура + влажность | I²C `0x44` | `air_temp`, `humidity` |
...
> **Датчик воздуха SHT30 (модуль GY-SHT30-D) ≠ SHT4x.** Адрес тот же (`0x44`),
> но в ESPHome это **другая платформа**: `sht3xd` (для SHT30/31/35), а не `sht4x`
> (для SHT40/41/45). В эталоне стоит `platform: sht4x` — при установке SHT30
> замените блок на `platform: sht3xd` (см. §7). Остальное (топики, MLX90614)
> не меняется.
...
### Распиновка платы (куда именно паять)

Оранжевым обведены пины, которые использует прошивка.

![Распиновка ESP32-C3 SuperMini](diagrams/10_esp32c3_pinout.svg)

> **Внимание — шёлкография обманчива.** На плате `SDA`/`SCL` подписаны на пинах
> **8 и 9**, но прошивка использует **GPIO5 (SDA)** и **GPIO6 (SCL)**. Пины 8/9
> на ESP32-C3 — strapping-пины (9 — это BOOT), на них I²C мешает прошивке,
> поэтому шина вынесена на 5/6. **Подключайте датчики к физическим пинам 5 и 6**,
> а не к подписанным SDA/SCL. (ESP32-C3 позволяет назначить I²C на любой GPIO.)

### Логическая схема соединений
...
### Датчик воздуха SHT30 вместо SHT4x (модуль GY-SHT30-D)

У SHT30/31/35 в ESPHome **другая платформа**, чем у SHT40/41/45. Если стоит
SHT30 — заменить в блоке датчика воздуха `platform: sht4x` на `platform: sht3xd`;
остальное (id `air_temp`/`humidity`, `internal: true`, `on_value → mqtt.publish`,
`update_interval`) не меняется. Адрес I²C тот же (`0x44`), топики те же. Сводка
платформ:

| Модуль (датчик) | Платформа ESPHome | I²C |
|---|---|---|
| GY-SHT30-D (SHT30/31/35) | `sht3xd` | `0x44` |
| SHT40 / SHT41 / SHT45 | `sht4x` | `0x44` |
| GY-906 (MLX90614) | `mlx90614` | `0x5A` |
