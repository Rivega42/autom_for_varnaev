"""Проверка Dockerfile сервиса log-service (E9.1): multi-stage, slim, non-root."""
...
_DOCKERFILE = REPO_ROOT / "services/log-service/Dockerfile"
...
def test_cmd_runs_asgi_app() -> None:
    """CMD запускает ASGI-приложение log_service.app:app через uvicorn."""
    content = _content()
    assert "uvicorn" in content
    assert "log_service.app:app" in content
